...
- (void)changeStatusBarOrientation:(NSInteger)orientation {
    ...
    _alertBackground.frame = viewController.view.bounds;
    alertViewContents.center = self.center;
}
...
#pragma mark - FrtcCAlertView Checks
...
- (void) checkCustomizationValid {
    ...
    if (![self hasSubTitle])
        if (![self hasTitle])
            NSLog(@"FrtcCAlertView Warning: Your Alert should have a title and/or subtitle.");
...
- (void)drawRect:(CGRect)rect {
    ...
    separatorLineView.backgroundColor = [UIColor colorWithRed:33 green:33 blue:33 alpha:1.0];
    // [UIColor colorWithWhite:100.0f/255.0f alpha:1.0]; // set color as you want.
    // if (_darkTheme)
    ...
    if (alertTextFields.count > 0) {
        ...
        for (int i = 0; i < MIN(alertTextFields.count,4); i++) {
            ...
            if (tf.text.length == 0 &&
                [[alertTextFields objectAtIndex:i] objectForKey:@"text"] != nil &&
                [[[alertTextFields objectAtIndex:i] objectForKey:@"text"] length] > 0)
                tf.text = [[alertTextFields objectAtIndex:i] objectForKey:@"text"];
    ...
    if (_numberOfButtons == 0) { // View only contains DONE/DISMISS Button
        ...
        horizontalSeparator.backgroundColor = UIColor.redColor;
    ...
    if (circleLayer.fillColor == [UIColor whiteColor].CGColor)
        [spinner setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleMedium];
    else
        [spinner setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleMedium];
...
{
    NSBundle *bundle = [NSBundle bundleWithURL:[[NSBundle bundleForClass:[self class]] URLForResource:@"FrtcCAlertView" withExtension:@"bundle"]];
    return bundle;
}
...
-(UIImage *)loadImageFromResourceBundle:(NSString *)imageName
{
    ...
    {
        NSBundle *bundle = [FrtcCAlertView getResourcesBundle];
...
- (void) showAlertView {
    ...
    id<FrtcCAlertViewDelegate> strongDelegate = self.delegate;
    
    if ([strongDelegate respondsToSelector:@selector(FrtcCAlertViewWillAppear:)]) {
        [strongDelegate FrtcCAlertViewWillAppear:self];
    }
...
- (void) dismissAlertView {
    ...
    [UIView animateWithDuration:0.175 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (!self->_animateAlertOutToTop && !self->_animateAlertOutToLeft && !self->_animateAlertOutToRight && !self->_animateAlertOutToBottom) {
            ...
            if ([strongDelegate respondsToSelector:@selector(FrtcCAlertViewDismissed:)]) {
                [strongDelegate FrtcCAlertViewDismissed:self];
            }
...
- (void)handleButton:(id)sender {
    ...
    if ([strongDelegate respondsToSelector:@selector(FrtcCAlertView:clickedButtonIndex:buttonTitle:)]) {
        [strongDelegate FrtcCAlertView:self clickedButtonIndex:[sender tag] buttonTitle:clickedButton.titleLabel.text];
    }
...
- (void)addTextFieldWithText:(NSString *)Text andTextReturnBlock:(FCTextReturnBlock)textReturn {
    if (textReturn != nil)
        [alertTextFields addObject:@{@"text" : Text,
                                     @"action" : textReturn}];
    else
        [alertTextFields addObject:@{@"text" : Text,
                                     @"action" : @0}];
