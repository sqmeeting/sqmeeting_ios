...
@interface FrtcHomeMeetingDetailViewController () <UITableViewDelegate,UITableViewDataSource,FrtcHomeMeetingDetailProtocol>
...
@implementation FrtcHomeMeetingDetailViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        FrtcHomeDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:KHomeMeetingDetailCellId forIndexPath:indexPath];
        ...
        if (!cell) {
            ...
            cancelBtn.frame = CGRectMake(KLeftSpacing, 10, KScreenWidth - KLeftSpacing * 2, 50);
...
- (void)didClickCellBtnAction:(NSIndexPath *)indexPath {
    if (indexPath.section == 1 && indexPath.row == 1) { //delegate meeting
        @WeakObj(self);
        [self showAlertWithTitle:NSLocalizedString(@"meeting_delete_m", nil)
                         message:NSLocalizedString(@"meeting_alert_delete", nil)
                    buttonTitles:@[NSLocalizedString(@"call_cancel", nil),NSLocalizedString(@"ok", nil)]
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[FrtcHomeDetailTableViewCell class]
           forCellReuseIdentifier:KHomeMeetingDetailCellId];
