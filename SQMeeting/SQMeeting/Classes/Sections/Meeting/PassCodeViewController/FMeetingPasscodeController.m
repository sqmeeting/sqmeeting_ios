...
@interface FMeetingPasscodeController ()<UIGestureRecognizerDelegate, UITextFieldDelegate, CustomTextFieldDeleteDelegate>

@end

@implementation FMeetingPasscodeController
...
- (void)viewDidLoad {
    ...
    [self.backGroundView setImage:[UIImage imageFromColor:KColorRGB(0, 0, 0, 1)]];
    [self.passcodeMainView setBackgroundColor:UIColor.whiteColor];
    
    NSLog(@"FMeetingPasscodeController The width is %f", self.view.frame.size.width);
...
- (UIImageView *)passcodeMainView {
    if (!_passcodeMainView) {
        ...
        _passcodeMainView.center = self.backGroundView.center;
...
- (void) checkPasscode {
    ...
    if (self.setPwdTextOne.text.length == 0) {
        ...
        //        NSLog(@"%@",passcode);
        //[[FrtcCall sharedCallClient] sendPasscode:passcode];
        [[FrtcCall sharedCallClient] sendPasscode:passcode];
...
- (void)customTextFieldDeleteBackward:(CustomTextField *)textField {
    NSLog(@"customTextFieldDeleteBackward");
    if (textField == self.setPwdTextFour) {
        NSLog(@"textFieldDidDeleteBackward setPwdTextFour");
        if ([textField.text isEqualToString:@""]) {
            [_setPwdTextThree becomeFirstResponder];
            _setPwdTextThree.text = @"";
        }
    } else if (textField == self.setPwdTextThree) {
        NSLog(@"textFieldDidDeleteBackward setPwdTextThree");
        if ([textField.text isEqualToString:@""]) {
            [_setPwdTextTwo becomeFirstResponder];
            _setPwdTextTwo.text = @"";
        }
    } else if (textField == self.setPwdTextTwo) {
        NSLog(@"textFieldDidDeleteBackward setPwdTextTwo");
        if ([textField.text isEqualToString:@""]) {
            [_setPwdTextOne becomeFirstResponder];
            _setPwdTextOne.text = @"";
        }
...
- (void)onClickHangUp:(UIButton*)sender {
    //[[SDKContext sharedSDKContext] dropCall:0];
    [[FrtcCall sharedCallClient] dropCall];
    [self dismissViewControllerAnimated:NO completion:nil];
}

//add by FRcom for hide status bar
