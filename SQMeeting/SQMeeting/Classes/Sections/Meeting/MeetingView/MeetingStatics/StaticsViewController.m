...
#import "FrtcStaticsCallTypeView.h"
...
@interface StaticsViewController () <UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong) FrtcStaticsCallTypeView *callView;
@property (nonatomic, strong) FrtcStaticsTagView *tagView;
...
- (void)configUI {
    ...
    [self.contentView addSubview:leftBarButton];
...
- (UITableView *)staticsTableView {
    if(!_staticsTableView) {
        ...
        if (@available(iOS 15.0, *)) {
            [_staticsTableView setSectionHeaderTopPadding:0.0f];
        }
    }
    return _staticsTableView;
}

- (FrtcStaticsCallTypeView *)callView {
    if (!_callView) {
        _callView = [[FrtcStaticsCallTypeView alloc]init];
        _callView.backgroundColor = KBGColor;
    }
    return _callView;
}

- (FrtcStaticsTagView *)tagView {
    if (!_tagView) {
        _tagView = [[FrtcStaticsTagView alloc]init];
