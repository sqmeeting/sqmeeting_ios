...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = NSLocalizedString(@"MEETING_LOG_UPLOADBUTTON", nil);
...
- (void)startUploadLog {
    ...
    ISMLog(@"uploadId = %td",self->uploadId);
...
- (void)handleUploadStatus:(BOOL)fileType {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        } else if (self->progressStatus == 100) {
            if (!fileType) {
                self->fileType = YES;
                return;
            }
...
- (void)goBackToSettingView {
    if (isLoginSuccess) {
        ...
        [self popToViewController:[SettingViewController class]];
    }
}
