#import "FrtcAccountUpdatePsdViewController.h"
#import "Masonry.h"
...
#define kTextField_Height 50

@interface FrtcAccountUpdatePsdViewController () <FrtcAccountProtocol,UITextFieldDelegate>
...
@implementation FrtcAccountUpdatePsdViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.title = NSLocalizedString(@"update_password", nil);
    //    self.oldPwdTextField.text = @"123456";
    //    self.pwdNewTextField.text = @"1234567";
    //    self.againNewPwdTextField.text = @"1234567";
}
...
- (void)saveButton:(UIButton*)sender {
    ...
    NSString *pwdold = self.againNewPwdTextField.text;
    if (kStringIsEmpty(pwdold)) {
        [MBProgressHUD showMessage:NSLocalizedString(@"password_low", nil)]; return;
    }
...
- (void)saveBtnEnabled {
    NSString *newPsd = self.pwdNewTextField.text;
    BOOL isEnabled = (newPsd.length >= 6 && self.againNewPwdTextField.text.length >= 6);
...
- (FAccountTextField *)getTextFieldWithLeftTitle:(NSString *)title placeholder:(NSString *)placeholder addBorder:(BOOL)isAddBorder{
    ...
    lelftLabel.backgroundColor = UIColor.whiteColor;
...
- (CGRect)textRectForBounds:(CGRect)bounds{
    ...
    CGFloat offset = 80 - textRect.origin.x;
