...
+ (NTESI420Frame *)pixelBufferToI420:(CVImageBufferRef)pixelBuffer
    ...
    libyuv::RotationModeEnum rotateMode = libyuv::kRotate90;
    
    if (i420Frame.height > i420Frame.width) { //iPad
        rotateMode = [NTESYUVConverter rotateMode:orientation];
        dstWidth = i420Frame.height;
        dstHeight = i420Frame.width;
        i420Frame.iPadPro = false;
    }else { //iPad Pro 、 iPad mini 、 iPad air
        rotateMode = [NTESYUVConverter iPadProRotateMode:orientation];
        dstWidth = i420Frame.width;
        dstHeight = i420Frame.height;
        i420Frame.iPadPro = true;
    }
    ...
    rotatedI420Frame.iPadPro = i420Frame.isIPadPro;
...
+ (libyuv::RotationModeEnum)iPadProRotateMode:(NTESVideoPackOrientation)orientation {
    switch (orientation) {
        case NTESVideoPackOrientationPortraitUpsideDown:
            NSLog(@"libyuv::kRotate180");
