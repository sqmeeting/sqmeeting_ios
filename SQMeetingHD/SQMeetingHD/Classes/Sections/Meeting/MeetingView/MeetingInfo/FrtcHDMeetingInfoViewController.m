...
#import "FrtcHDMeetingInfoLeftView.h"
#import "FrtcHDMeetingNetWorkView.h"
...
@interface FrtcHDMeetingInfoViewController () <TYTabPagerViewDataSource, TYTabPagerViewDelegate>

@property (nonatomic, strong) UISegmentedControl *segmentedControl;
@property (nonatomic, strong) UILabel *titleLabel;
@property (nonatomic, strong) UIView *bgView;
@property (nonatomic, strong) UIView *crossBarView;
@property (nonatomic, strong) FrtcHDMeetingInfoLeftView *infoLeftView;
@property (nonatomic, strong) FrtcHDMeetingNetWorkView *staticsView;
@property (nonatomic, strong) TYTabPagerView *pagerView;

@end

@implementation FrtcHDMeetingInfoViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updataMediaStaticsInfo:) name:FMeetingInfoMediaStaticsInfoNotNotification object:nil];
}
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:FMeetingInfoMediaStaticsInfoNotNotification object:nil];
}
...
- (void)meetingInfoViewLayout {
    ...
    [self.bgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(LAND_SCAPE_HEIGHT * 0.45);
...
- (UIView *)tabPagerView:(TYTabPagerView *)tabPagerView viewForIndex:(NSInteger)index prefetching:(BOOL)prefetching {
    if (index == 1) {
        FrtcHDMeetingNetWorkView *networkView = (FrtcHDMeetingNetWorkView *)[tabPagerView dequeueReusablePagerCellWithReuseIdentifier:@"FrtcHDMeetingNetWorkView" forIndex:index];
        networkView.backgroundColor = UIColor.clearColor;
        @WeakObj(self);
        networkView.staticsInfoCallBack = ^{
            @StrongObj(self)
            FrtcHDStaticsViewController *staticsViewController = [[FrtcHDStaticsViewController alloc]init];
        ...
        FrtcHDMeetingInfoLeftView *infoView = (FrtcHDMeetingInfoLeftView *)[tabPagerView dequeueReusablePagerCellWithReuseIdentifier:@"FrtcHDMeetingInfoLeftView" forIndex:index];
...
- (TYTabPagerView *)pagerView {
    if (!_pagerView) {
        ...
        [_pagerView registerClass:[FrtcHDMeetingInfoLeftView class] forPagerCellWithReuseIdentifier:@"FrtcHDMeetingInfoLeftView"];
        [_pagerView registerClass:[FrtcHDMeetingNetWorkView class] forPagerCellWithReuseIdentifier:@"FrtcHDMeetingNetWorkView"];
