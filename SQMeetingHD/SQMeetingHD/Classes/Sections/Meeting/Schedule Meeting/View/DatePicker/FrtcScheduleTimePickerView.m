...
+ (void)showTimePickerViewWithTimeStr:(TimePickerViewBlock)datePickerBlock {
    ...
    [[FrtcHelpers getCurrentVC].view addSubview:timePickerView];
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithActionBlock:^(id  _Nonnull sender) {
        [timePickerView disMiss];
    }];
    tap.delegate = timePickerView;
    [timePickerView addGestureRecognizer:tap];
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat left_spacing = self.pickerView.bounds.size.width / 3 + 60;
        [self.dateLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.pickerView);
            make.left.mas_equalTo(left_spacing);
        }];
        
        CGFloat time_left_spacing = self.pickerView.bounds.size.width * 0.98 + 90;
        [self.timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.pickerView);
            make.left.mas_equalTo(time_left_spacing);
        }];
    }
    return self;
}
...
- (UILabel *) dateLabel {
    if (!_dateLabel) {
        ...
        _dateLabel.text = NSLocalizedString(@"hour", nil);;
