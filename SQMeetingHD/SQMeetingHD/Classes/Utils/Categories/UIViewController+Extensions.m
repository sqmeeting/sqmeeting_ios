...
- (void)showTextFieldAlertWithTitle:(NSString *)title
    ...
    switch (style) {
        ...
        case FTextFieldRenamed:
            alertactionStr = NSLocalizedString(@"string_done", nil);
            break;
        default:
            break;
    }
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        switch (style) {
            ...
            case FTextFieldRenamed:
            {
                textField.text = [FrtcLocalInforDefault getMeetingDisPlayName];
            }
            default:
                break;
        }
...
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"weiza_iphone" message:@"" preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction *cancelAction1 = [UIAlertAction actionWithTitle:NSLocalizedString(@"dialog_cancel", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    }];
    [alertController addAction:cancelAction1];
    
    UIAlertAction *cancelAction2 = [UIAlertAction actionWithTitle:NSLocalizedString(@"dialog_cancel", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    }];
    [alertController addAction:cancelAction2];
    
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"call_cancel", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction *act){
    }];
    [alertController addAction:cancelAction];
    [self getSubView:alertController.view andLevel:1];
...
- (void)getSubView:(UIView *)view andLevel:(int)level {
    ...
    for (UIView *subView in subViews) {
        ...
        if ([subView isKindOfClass:[UILabel class]]) {
            UILabel *ulll = (UILabel *)subView;
            if (ulll.text.length > 3) {
                ulll.textAlignment = NSTextAlignmentLeft;
            }
        }
        [self getSubView:subView andLevel:(level + 1)];
    }
}

- (void)presentHDViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    CGSize size = CGSizeMake(kIPAD_WIDTH,LAND_SCAPE_HEIGHT-100);
    [viewControllerToPresent setModalPresentationStyle:UIModalPresentationFormSheet];
    viewControllerToPresent.preferredContentSize = size;
    PYHDNavigationController *nav = [[PYHDNavigationController alloc]initWithRootViewController:viewControllerToPresent];
