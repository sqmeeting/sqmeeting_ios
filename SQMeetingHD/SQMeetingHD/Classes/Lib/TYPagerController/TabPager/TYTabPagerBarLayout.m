...
- (void)transitionFromCell:(UICollectionViewCell<TYTabPagerBarCellProtocol> *)fromCell toCell:(UICollectionViewCell<TYTabPagerBarCellProtocol> *)toCell animate:(BOOL)animate {
    ...
    void (^animateBlock)() = ^{
        if (fromCell) {
            fromCell.titleLabel.font = [UIFont systemFontOfSize:16.f];/// _normalTextFont;
        ...
        if (toCell) {
            toCell.titleLabel.font = [UIFont systemFontOfSize:16.f]; //_normalTextFont;
...
- (void)setUnderLineFrameWithIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    CGFloat progressY = 48; //_barStyle == TYPagerBarStyleCoverView ? (cellFrame.size.height - _progressHeight)/2:(cellFrame.size.height - _progressHeight - _progressVerEdging);
...
- (void)setUnderLineFrameWithfromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress
{
    ...
    CGFloat progressY = 48;//_barStyle == TYPagerBarStyleCoverView ? (toCellFrame.size.height - _progressHeight)/2:(toCellFrame.size.height - _progressHeight - _progressVerEdging);
